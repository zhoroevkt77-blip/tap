...
SYNS = [
    ...
    ["батир", "квартира"],
    ["там", "үй", "дом"],
    ["жер", "участок", "участка", "тилке"],
    ["дүкөн", "магазин", "бутик"],
    ["кеңсе", "офис"],
    ["кампа", "склад"],
    ["гараж", "бокс"],
    ["ижара", "аренда", "жалдоо", "арендага"],
    ["кыймылсыз", "недвижимость"],
    # ── Унаа ──
    ["унаа", "машина", "авто", "автомобиль"],
    # BRAND_SYNS: марканын кирилл жана латын жазылышы
    ["toyota", "тойота"],
    ["lexus", "лексус"],
    ["mercedes", "мерседес", "мерс", "бенз"],
    ["honda", "хонда"],
    ["hyundai", "хендай", "хундай", "хёндэ"],
    ["nissan", "ниссан"],
    ...
    ["porsche", "порше"],
    ["land", "ленд", "лэнд"],
    ["cruiser", "крузер", "круизер"],
    ["camry", "камри"],
    ["prius", "приус"],
    ["кыймылдаткыч", "двигатель", "мотор"],
    ["тетик", "запчасть", "запчасти"],
    ["дөңгөлөк", "резина", "шина", "колесо"],
    ["айдоочу", "водитель", "шофер"],
    # ── Техника ──
    ...
    ["кир жуугуч", "стиральная", "стиралка"],
    ["эмерек", "мебель"],
    # ── Кийим ──
    ["кийим", "одежда"],
    ["бут кийим", "обувь"],
    ["балдар", "детские", "детский"],
    # ── Мал жана айыл чарба ──
    ["мал", "скот", "жаныбар", "животное"],
    ["уй", "корова"],
    ["кой", "овца", "баран"],
    ["жылкы", "лошадь"],
    ["эчки", "коза"],
    ...
    ["бал", "мед"],
    # ── Кызмат жана жумуш ──
    ["жумуш", "работа", "вакансия"],
    ["курулуш", "стройка", "ремонт", "оңдоо"],
    ["жеткирүү", "доставка"],
    ["ташуу", "перевозка", "грузоперевозка"],
    ["окуу", "курс", "курстар", "обучение"],
    ["котормо", "перевод"],
    # ── Соода сөздөрү ──
    ["сатылат", "продается", "продам", "сатам"],
    ["алмашуу", "обмен"],
    ["соода", "торг"],
...
# Азыр төрт вариант: 1, 2, 3, 4 апта. Эскилери да калды —
# мурда коюлган жарыялар туура эсептелсин.
DURATION_DAYS = {
    "15 күн": 15, "10 күн": 10, "7 күн": 7, "5 күн": 5, "3 күн": 3,
    "1 күн": 1,
    # ── эски маанилер (жаңы жарыяларда сунулбайт) ──
    "4 апта": 28, "3 апта": 21, "2 апта": 14, "1 апта": 7,
    "3 ай": 90, "2 ай": 60, "1 ай": 28,
}

# Мөөнөт тандалбаса — ушунча күн
DEFAULT_DAYS = 7


def expiry_from(duration, days=None):
    """Мөөнөттүн бүтөр күнү. Тандалбаса — DEFAULT_DAYS."""
    if days is None:
        d = str(duration or "").strip().lower()
        days = DEFAULT_DAYS
        # Узунунан кыскасына карай текшеребиз: «4 апта (1 ай)» дегенде
        # «1 ай» деген бөлүгү мурда табылып калбашы үчүн.
        for k in sorted(DURATION_DAYS, key=len, reverse=True):
            if k.lower() in d:
                days = DURATION_DAYS[k]
                break
...
def _filters(q=None, cat=None, region=None, sub=None,
    ...
    if q and q.strip():
        ...
        # Ар бир сөзгө синонимдери да кошулат, ошондуктан «квартира» деп
        # изделсе «батир» деп жазылган эски жарыя да табылат.
        for w in [x for x in str(q).lower().split() if len(x) > 1][:5]:
            forms = {w, norm(w)}
            for g in SYNS:
                if any(w == s or (len(s) >= 4 and w.startswith(s)) for s in g):
                    for s in g:
                        forms.add(s)
                        forms.add(norm(s))
            forms = [f for f in forms if f]
            sql += " AND (" + " OR ".join(["stext LIKE ?"] * len(forms)) + ")"
            p += [f"%{f}%" for f in forms]
    return sql, p
...
def revive(lid, tg_id, days=15, phone=None):
