...
MSG = {
    ...
    "del_btn":    ("✅ Сатылды / Жабуу",            "✅ Продано / Закрыть"),  #ACTUAL1
    ...
    "ref_share_t": ("ТАП! — Кыргызстандагы акылдуу жарыя платформасы. "
                    "Телефонго эч нерсе орнотуунун кереги жок!",
                    "ТАП! — умная платформа объявлений Кыргызстана. "
                    "Ничего устанавливать не нужно!"),
    "ref_got":    ("🎉 Досуңуз биринчи жарыясын койду! Жалпы дос: <b>%d</b>.\n"
    ...
    "ref_hello":  ("👋 Досуңуз сизди ТАП!ке чакырды. Кош келиңиз!\n\n"
                   "🎁 Биринчи жарыяңызды койгонуңузда сизге да, "
                   "досуңузга да кошумча жарыя кошулат.",
                   "👋 Друг пригласил вас в ТАП!. Добро пожаловать!\n\n"
                   "🎁 Когда вы разместите первое объявление, "
                   "дополнительные объявления получите и вы, и ваш друг."),
    "ref_thanks": ("🎁 Досуңуздун чакыруусу үчүн сизге "
                   "<b>+%d кошумча жарыя</b> кошулду!",
                   "🎁 За приглашение друга вам добавлено "
                   "<b>+%d дополнительных объявления</b>!"),
...
def save_ad(chat, uid, name, u):
    ...
    try:
        rok, inv_id, rn, radd, rmine = core.claim_referral(uid)
    except Exception as e:
        print("  Чакыруу бонусу катасы:", e, flush=True)
        rok = False
    if rok:
        send(chat, m("ref_thanks", lang, rmine))
        try:
            send(int(inv_id), m("ref_got", lang, rn, radd))
        except Exception:
    ...
    if text.startswith("/start") or text == "/help":
        ...
        if payload.startswith("ref") and payload[3:].isdigit():
            try:
                ...
                ok = False
            if ok:
                # Бонус азыр эмес — досу биринчи жарыясын койгондо берилет
